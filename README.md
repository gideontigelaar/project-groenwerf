...
A grass height measurement system for ride-on mowers, utilizing a Raspberry Pi Pico 2W to process sensor data and transmit it to a remote server. The system features a secure dashboard and optimized routing insights.
...
- **`api/`**: Flask-based REST API designed for production deployment (Ubuntu). Receives JSON payloads from the firmware, handles ArcGIS synchronization, manages the MySQL database, and includes `generate_route.py` for automated route processing.
