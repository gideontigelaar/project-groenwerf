...
void VibrationProcessor::updateAccel(float accel_z, float dt_s) {
    ...
    _rms_accum = Config::Sensor::RMS_ALPHA * _rms_accum + (1.0f - Config::Sensor::RMS_ALPHA) * (hpf * hpf); // square signal and apply exponential moving avg
...
uint16_t VibrationProcessor::compensate(uint16_t sonic_mm) const {
    ...
    _last_was_spike = false;
    _spike_hold_count = 0; // clear hold count on clean reading
